...
handle_info({tcp_error, _Socket, Reason}, State) ->
    ?PRINT("tcp_error~n", []),
    Status = "00 tcp error  Abnormal Disconnect",
    connector_event_server:gateway_status_log(State#state.gatewayId, Status),
    update_gateway_status(State#state.gatewayId, State#state.gatewayType, ?OFFLINE_STATUS),
    {stop, Reason, State};
...
handle_info({tcp, Socket, Packet = <<16#FA, 16#07, 16#13, 16#02, 16#FA, 16#02,
    ...
    case connector_gateway_type_store:lookup(GatewayId) of
        {error, _} ->
...
handle_info({tcp, Socket, <<0>>}, State) ->
    ...
    connector_active_report:active_report_645_packet(ActiveReportPid, <<16#00>>),
                            ...
                            {IsLogin, IsHeartBeat} when (IsLogin =:= true) orelse (IsHeartBeat =:= true) ->
                                case IsLogin of
                                    ...
                                    false ->
                                        ...
                                        case check_gateway_time(FrameHex) of
                                            true ->
                                                ok;
                                            false ->
                                                HeartResponse = get_check_time_response(FrameHex),
                                                ok = Transport:send(Socket, HeartResponse),
                                                ?PRINT("Concentrator respon frame:~p~n", [hex_util:to_hex(HeartResponse)])
                                        end,
...
%%检查集中器时间，判断是否需要重置集中器时间
check_gateway_time(FrameHex) ->
    TimeStrTmp = string:sub_string(FrameHex, 37, 48),
    TimeStr = ?HELP:get_reverse_timestr(TimeStrTmp),
    %%此处的时间数据是字符串格式，跟当前时间比较需要作类型转换
    [Year,Month,Data,Hour,Min,Sec] = [string:substr(TimeStr,X,2) || X <- lists:seq(1,12,2)],
    {{Y, M, D},{H, Mi, S}} = calendar:local_time(),
    GatewayTime = list_to_integer(Data)*86400 + list_to_integer(Hour)*3600 + list_to_integer(Min)*60 + list_to_integer(Sec),
    OsTime = D*86400 + H*3600 + Mi*60 + S,
    case Year == string:right(integer_to_list(Y), 2) of
         true ->
             case change_type(Month) == M of
                 true ->
                     case abs(GatewayTime - OsTime) > 180 of
                         true ->
                              false;
                         _ ->
                              true
...
%%根据集中器心跳包获取集中器重置时间报文
get_check_time_response(FrameHex) ->
    Start = "68",
    Len = "a200",
    Flag0 = "68",
    Ctr = "40",
    Address = string:sub_string(FrameHex,15,24),
    Flag1 = "05f000004003",
    {T = {Year, Month, Day}, {Hour, Min, Sec}} = calendar:local_time(),
    D0 = string:right(integer_to_list(Year), 2),
    %%计算星期几
    Week = calendar:day_of_the_week(T),
    Part1 = ?HELP:get_complete_data(integer_to_list(Week, 2), "0", 3), 
    Part2 = case Month < 10 of
              true ->
                  ?HELP:get_complete_data(integer_to_list(Month,2), "0", 5);
              _ ->
                  lists:concat(["1", ?HELP:get_complete_data(integer_to_list(Month-10, 2),"0",4)])
            end,
    MonthBinStr = Part1++Part2,
    D1 = integer_to_list(list_to_integer(string:left(MonthBinStr,4),2),16) ++ integer_to_list(list_to_integer(string:right(MonthBinStr,4),2),16),
    D2 = ?HELP:get_complete_data(integer_to_list(Day),"0", 2),
    D3 = ?HELP:get_complete_data(integer_to_list(Hour),"0", 2),
    D4 = ?HELP:get_complete_data(integer_to_list(Min),"0", 2),
    D5 = ?HELP:get_complete_data(integer_to_list(Sec),"0", 2),
    DateTime = lists:concat([D5, D4, D3, D2, D1, D0]), 
    Flag2 = "00000000000000000000000000000000",
    Flag3 = "00",
    Time = lists:concat([D5, D4, D3, D2, "00"]),
    DataField = lists:concat([Ctr, Address, Flag1, DateTime, Flag2, Flag3, Time]),
    Cs = ?HELP:get_cs(DataField),
    Stop = "16",
    ResponFrame = lists:concat([Start,Len,Len,Flag0,DataField,Cs,Stop]),
    ?PRINT("Response:~p~n",[ResponFrame]),
    hex_util:to_bin(ResponFrame).


change_type(DataHex) ->
    DataStr = integer_to_list(list_to_integer(DataHex, 16), 2),
    DataBinStr = ?HELP:get_complete_data(DataStr, "0", 8),
    DataTmp = string:substr(DataBinStr, 4, 1) ++ integer_to_list(list_to_integer(string:right(DataBinStr,4),2)),
    list_to_integer(DataTmp).
